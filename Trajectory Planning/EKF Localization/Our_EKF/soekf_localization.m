...
% Any questions please contact: zjuyinhuan@gmail.com
    ...
    for i=1 : nSteps
        ...
        hF = hessF(xEkf, u); % 计算海森矩阵
        so = 0;
        for i=1:3
           for j=1:3
               ei = zeros(3,1);ei(i) = 1;
               ej = zeros(3,1);ej(j) = 1;
               so = so + ei*ej'*trace(hF{i}*PxEkf*hF{j}*PxEkf); % 计算二次项
           end
        end
        PxEkfPred = G * PxEkf * G' + so +convQ;  % 计算协方差预测值
...
function hF = hessF(x, u)
    global dt;
    % -----------------------------
    v = sqrt(u(1)^2+u(2)^2);
    w = u(3);
    hF{1}=[0,0,0;0,0,0;0,0,v/w*sin(x(3))-v/w*sin(x(3)+w*dt)];
    hF{2}=[0,0,0;0,0,0;0,0,-v/w*cos(x(3))+v/w*cos(x(3)+w*dt)];
    hF{3}=zeros(3,3);
%     syms xt yt theta;
%     a = [xt yt theta];
%     f = [xt-v/w*sin(theta)+v/w*sin(theta+w*dt);
