...
title('PPICP with KD-tree and Deleter点云地图与定位轨迹');

figure; % 绘制定位轨迹俯视图
plot(x,y,'r*-','LineWidth',1.2);
xlabel('x');
ylabel('y');
title('PPICP with KD-tree and Deleter定位轨迹俯视图');
grid on;
...
function [p_match,dist] = pointMatch_knn(p,pp)
...
function [p_match,dist] = pointMatch_dist(p,pp) 
...
for i=1:size(pp,1)
    ...
    [dist(i),min_index]=min(distance_square); % 找到距离最近的点
...
function [p_temp,pp_temp] = deleteBadpoints(p,pp,dist,threshold)
% 剔除距离过大的点
% 输入参数：
% p: 点集合P
% pp: 点集合P'
...
% threshold：判决阈值
% 输出值：
% p_temp：剔除距离过大的点后的点集合P
% pp_temp：剔除距离过大的点后的点集合P'

% inlier_ratio = 1.0;
inlier_num = sum((dist<threshold)); % 计算在距离范围内的点个数
[~, idx] = sort(dist); % 排序
% inlier_num = round(size(pp,1)*inlier_ratio);
idx = idx(1:inlier_num);
pp_temp = pp(idx,:);
p_temp = p(idx,:); % 选出符合条件的点
end
...
% 根据最近邻域规则建立p和pp的关系
[p_match,dist] = pointMatch_knn(p,pp); % KD-tree加速搜索
% [p_match,dist] = pointMatch_dist(p,pp); % 欧式距离搜索

for i=1:i_max % 最大迭代次数
    
    % 剔除距离过大的点
    [p_temp,pp_temp] = deleteBadpoints(p_match,pp,dist,2);
    
    % SVD分解，估计旋转平移量
    [R,t] = svdSolution(p_temp,pp_temp);
